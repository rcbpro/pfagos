...
if(file_exists($pre_img_file_path)){
	...
	$_SESSION['pre_pfac_preview_img']['file_path'] = $pre_img_file_path;
}
...
//				check_to_upload();
	...
	*/
			   ...
			   success: function(resdata){
				   if(resdata){
					  $("#txt_url_upload").attr("disabled","disabled");
					   ...
					   $("#txt_url_upload").removeAttr("disabled").focus().val("");
				   }
			   }
		});
	}
	...
	function remove_image_current(){
		$.ajax({type: "POST",url: "../../../lib/uploader/uploader-proccess.php",data: "remove-img-current=true",
			   success: function(resdata){
		...
		<div id="url-upload">
            ...
            <?php
				...
				if(!is_file($pre_video_file_path) && !is_file($_SESSION['pfac_preview_video']['file_path_video'])){
					if(strstr($_SESSION['pre_pfac_preview_video']['name'],'http://')){
...
<form name="pfac_media_form2" action="" method="post">
    <input type="button" id="pfac_media_submit_preview" name="pfac_media_submit_preview" value="Preview" class="inputs submit" onclick="javascript:popup('<?php echo $site_config['base_url'];?>views/previews/preview-pfa-edit.php');" />            
...
//	echo '<pre>';
//	print_r($_SESSION['pre_pfac_preview_img']);
//	echo '</pre>';
//	echo '</div>';
?>